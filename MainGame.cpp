...
void HandlePlayerControls()
{
	...
	if (Play::KeyDown(VK_UP))
	{
		...
		Play::SetSprite(obj_agent8, "agent8_fall", 0.25f);
	}
	...
	if (Play::KeyPressed(VK_SPACE))
	{
		...
		int id = Play::CreateGameObject(TYPE_LASERS, firePos, 30, "laser");
...
void UpdateCoinsAndStars() //implementing coins & stars
{
	GameObject& obj_agent8 = Play::GetGameObjectByType(TYPE_AGENT8);
...
void UpdateLasers()
{
	std::vector<int> vLasers = Play::CollectGameObjectIDsByType(TYPE_LASERS);
	std::vector<int> vTools = Play::CollectGameObjectIDsByType(TYPE_TOOL);
	std::vector<int> vCoins = Play::CollectGameObjectIDsByType(TYPE_COIN);
